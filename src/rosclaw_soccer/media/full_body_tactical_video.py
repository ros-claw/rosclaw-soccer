"""Evidence-downstream video for the learned three-G1 tactical bridge."""
...
_CLAIM = "FULL_BODY_G1_TACTICAL_BRIDGE_REPLAY"


def render_full_body_tactical_video(
    *,
    evidence_dir: Path,
    asset_root: Path,
    output_path: Path,
    ...
    """Render one learned pass and shot; pixels never enter any gate."""
    ...
    if (
        ...
        raise ValueError("full-body tactical video output contract is invalid")
    validation = validate_full_body_tactical_growth_stage(root, source_checkout=checkout)
    if validation.get("status") != "VALIDATED_FULL_BODY_TACTICAL_STAGE":
        raise ValueError("full-body tactical evidence did not validate")
    report_path = root / "retention/retention-exam.json"
    actor_path = root / "full-body-tactical-actor.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    rows = report.get("rows")
    if not isinstance(rows, list):
        raise ValueError("full-body retention rows are missing")
    selected = {}
    for action in ("pass", "shoot"):
        selected[action] = next(
            (
                (index, row)
                for index, row in enumerate(rows)
                if isinstance(row, dict)
                and row.get("selected_action") == action
        ...
        if selected[action] is None:
            raise ValueError(f"retention has no safe learned {action}")
    trajectories: dict[str, dict[str, np.ndarray]] = {}
    source_files = {
        str(report_path): hash_bytes(report_path.read_bytes()),
        str(actor_path): hash_bytes(actor_path.read_bytes()),
    }
    labels: dict[str, str] = {}
    required = {
        "time",
    ...
    for action, item in selected.items():
        index, row = cast(tuple[int, dict[str, Any]], item)
        artifact = row["primary_artifact"]
        path = root / "retention" / f"case-{index:03d}" / artifact["file"]
        if hash_bytes(path.read_bytes()) != artifact["file_hash"]:
            raise ValueError("full-body video trajectory file changed")
        with np.load(path, allow_pickle=False) as archive:
            trajectory = {key: np.asarray(archive[key]) for key in archive.files}
        if (
            required - trajectory.keys()
            or trajectory_digest(trajectory) != artifact["trajectory_digest"]
        ):
            raise ValueError("full-body video trajectory binding changed")
        trajectories[action] = trajectory
        source_files[str(path)] = hash_bytes(path.read_bytes())
        result = row["primary_result"]
        if action == "pass":
            labels[action] = (
                "PRESS READ · LEARNED PASS · FOOT RECEPTION "
                f"{float(result['teammate_foot_reception_distance_m']):.3f} m"
            )
        else:
            labels[action] = (
                "COVER READ · LEARNED SHOOT · GOAL ("
                f"y={float(result['goal_crossing_y_m']):+.2f}, "
                f"z={float(result['goal_crossing_z_m']):.2f} m)"
            )
    clips = _timeline(trajectories, labels, fps)
    ffmpeg = shutil.which("ffmpeg")
    ffprobe = shutil.which("ffprobe")
    if ffmpeg is None or ffprobe is None:
        raise RuntimeError("ffmpeg and ffprobe are required for full-body tactical video")
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="rosclaw-full-body-2v1-") as temp:
                ...
                if process.stdin is None:
                    raise RuntimeError("full-body tactical raw-video pipe is unavailable")
                try:
                    _write_frames(
                        mujoco=mujoco,
                        model=model,
                ...
                if process.wait():
                    raise RuntimeError(f"full-body tactical ffmpeg failed: {stderr[-3000:]}")
        finally:
            renderer.close()
    finally:
    ...
    if (
        ...
        raise RuntimeError("full-body tactical encoded video contract changed")
    manifest: dict[str, Any] = {
        "schema_version": "rosclaw_soccer.full_body_tactical_video.v1",
        "video_path": str(output),
        "video_hash": hash_bytes(output.read_bytes()),
        "source_files": source_files,
        "source_stage_hash": validation["stage_hash"],
        "source_actor_hash": validation["actor_hash"],
        "claim": _CLAIM,
        "strict_replay": True,
        "learned_actions_shown": ["pass", "shoot"],
    ...
    validate_full_body_tactical_video_manifest(manifest_path)
    return manifest
...
def _timeline(
    ...
    clips: list[_Clip] = []
    for action in ("pass", "shoot"):
        end = float(np.asarray(trajectories[action]["time"])[-1])
        clips.append(
            _Clip(
                label=labels[action],
                frames=_segment(action, 2.00, min(6.50, end), 0.75, "wide", fps),
            )
        )
        clips.append(
            _Clip(
                label=(
                    "CONTACT REPLAY · CAUSAL BODY INTERACTION · THREE 29-DoF G1"
                    if action == "pass"
                    else "CONTACT REPLAY · ONE SHARED CPU MUJOCO SOLVER · SIM ONLY"
                ),
                frames=_segment(action, 4.70, min(6.50, end), 0.55, "chain", fps),
            )
        )
    return tuple(clips)
...
def _ffmpeg_command(
    *,
    ...
    filters = [
        ...
        f"drawtext={font_option}text='ROSClaw Soccer · LEARNED FULL-BODY 2v1':"
        f"expansion=none:x={left}:y={round(13 * scale)}:fontsize={round(32 * scale)}:"
        "fontcolor=white",
        f"drawtext={font_option}text='PASS / SHOOT ACTOR · PHYSICAL CONTACT SCORING · "
...
def validate_full_body_tactical_video_manifest(path: Path) -> dict[str, Any]:
    """Verify the encoded replay and all evidence bindings."""

    payload = json.loads(path.expanduser().resolve().read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("full-body tactical video manifest must be an object")
    expected = payload.pop("manifest_hash", None)
    if expected != hash_json(payload):
        raise ValueError("full-body tactical video manifest integrity mismatch")
    video_value = payload.get("video_path")
    source_files = payload.get("source_files")
    if not isinstance(video_value, str) or not isinstance(source_files, dict):
        raise ValueError("full-body tactical video bindings are invalid")
    video = Path(video_value).expanduser().resolve()
    if not video.is_file() or hash_bytes(video.read_bytes()) != payload.get("video_hash"):
        raise ValueError("full-body tactical video hash changed")
    for source_value, source_hash in source_files.items():
        ...
        if not source.is_file() or hash_bytes(source.read_bytes()) != source_hash:
            raise ValueError("full-body tactical video source binding changed")
    numbers = tuple(
        payload.get(name) for name in ("fps", "width", "height", "frame_count", "duration_sec")
    )
    if (
        payload.get("schema_version") != "rosclaw_soccer.full_body_tactical_video.v1"
        or payload.get("claim") != _CLAIM
        or payload.get("learned_actions_shown") != ["pass", "shoot"]
        ...
        raise ValueError("full-body tactical video authority contract is invalid")
