"""Evidence-bound slow-motion reel for the S89 late-lunge tip-over.

The renderer consumes a frozen CPU-MuJoCo trajectory after qualification.  It
cannot change physics or promotion state, and the manifest keeps the honest
``not airborne`` claim visible to downstream consumers.
"""
...
_CLAIM = "STABLE_LATE_LUNGE_TIP_OVER_NOT_AIRBORNE_DIVE"


def validate_dynamic_aerial_lunge_video_manifest(path: Path) -> dict[str, Any]:
    """Verify video, evidence, trajectory and authority bindings."""

    payload = json.loads(path.expanduser().resolve().read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("dynamic aerial lunge video manifest must be an object")
    expected = payload.pop("manifest_hash", None)
    if expected != hash_json(payload):
        raise ValueError("dynamic aerial lunge video manifest integrity mismatch")
    video_value = payload.get("video_path")
    source_files = payload.get("source_files")
    if not isinstance(video_value, str) or not isinstance(source_files, dict):
        raise ValueError("dynamic aerial lunge video bindings are invalid")
    video = Path(video_value).expanduser().resolve()
    if not video.is_file() or hash_bytes(video.read_bytes()) != payload.get("video_hash"):
        raise ValueError("dynamic aerial lunge video hash changed")
    for source_value, source_hash in source_files.items():
        ...
        if not source.is_file() or hash_bytes(source.read_bytes()) != source_hash:
            raise ValueError("dynamic aerial lunge video source binding changed")
    numbers = tuple(
        payload.get(name) for name in ("fps", "width", "height", "frame_count", "duration_sec")
    )
    if (
        payload.get("schema_version") != "rosclaw_soccer.dynamic_aerial_lunge_video.v1"
        ...
        or payload.get("airborne_claimed") is not False
        ...
        raise ValueError("dynamic aerial lunge video authority contract is invalid")
    payload["manifest_hash"] = expected
    return cast(dict[str, Any], payload)


def render_dynamic_aerial_lunge_video(
    *,
    evidence_path: Path,
    asset_root: Path,
    output_path: Path,
    ...
    """Render the passed S89 replay without using pixels for scoring."""
    ...
    if (
        ...
        raise ValueError("dynamic aerial lunge video output contract is invalid")
    ...
    metrics = replay.get("dynamic_metrics") if isinstance(replay, dict) else None
    ...
    if not (
        ...
        and metrics.get("airborne_claimed") is False
        and isinstance(result, dict)
        and result.get("goalkeeper_save_observed") is True
        and result.get("goal_crossed") is False
        and result.get("goalkeeper_glove_contact_surface_distance_m") is not None
    ):
        raise ValueError("dynamic aerial lunge evidence is not render eligible")
    ...
    if not isinstance(trajectory_name, str) or Path(trajectory_name).name != trajectory_name:
        raise ValueError("dynamic aerial lunge trajectory name is invalid")
    ...
    if (
        ...
        raise ValueError("dynamic aerial lunge evidence bindings changed")
    ...
    if not isinstance(goal_value, dict) or not isinstance(config_value, dict):
        raise ValueError("dynamic aerial lunge request is incomplete")
    goal = G1TrainingGoalSpec(**goal_value)
    if abs(goal.width_m - 7.32) > 1e-9 or abs(goal.height_m - 2.44) > 1e-9:
        raise ValueError("dynamic aerial lunge video requires a regulation goal")
    ...
    if not required <= set(trajectory):
        raise ValueError("dynamic aerial lunge trajectory is incomplete")
    clips = _timeline(replay, trajectory, fps)
    ffmpeg = shutil.which("ffmpeg")
    ffprobe = shutil.which("ffprobe")
    if ffmpeg is None or ffprobe is None:
        raise RuntimeError("ffmpeg and ffprobe are required for dynamic aerial lunge video")
    ...
    try:
        ...
        if qualification.body_hash != request.get("body_hash"):
            raise ValueError("dynamic aerial lunge Body hash changed")
        lane = config_value.get("lane")
        aerial = config_value.get("aerial_config")
        if not isinstance(lane, dict) or not isinstance(aerial, dict):
            raise ValueError("dynamic aerial lunge lane contract is incomplete")
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="rosclaw-dynamic-lunge-") as temp_text:
                ...
                if process.stdin is None:
                    raise RuntimeError("dynamic aerial lunge raw-video pipe is unavailable")
                try:
                    _write_frames(
                        mujoco=mujoco,
                        model=model,
                ...
                if process.wait():
                    raise RuntimeError(f"dynamic aerial lunge ffmpeg failed: {stderr[-3000:]}")
        finally:
            renderer.close()
    finally:
    ...
    if (
        ...
        raise RuntimeError("dynamic aerial lunge encoded video contract changed")
    surface_distance = float(result["goalkeeper_glove_contact_surface_distance_m"])
    manifest: dict[str, Any] = {
        ...
        "glove_surface_distance_m": surface_distance,
        "glove_contact_height_m": result["goalkeeper_glove_contact_height_m"],
        "post_save_recovered": True,
        "recovery_maximum_linear_speed_mps": metrics["recovery_maximum_linear_speed_mps"],
    ...
    validate_dynamic_aerial_lunge_video_manifest(manifest_path)
    return manifest
...
def _timeline(
    ...
    metrics = replay["dynamic_metrics"]
    ...
    intro = tuple(_Frame(max(start, pass_time - 0.8), "wide") for _ in range(round(1.2 * fps)))
    continuous = (
        *_segment(max(start, pass_time - 0.8), shot_time - 0.35, 1.0, "pass", fps),
        *_segment(shot_time - 0.35, save_time + 0.85, 1.0, "hero", fps),
    )
    lunge = _segment(shot_time - 0.25, save_time + 0.55, 0.28, "hero", fps)
    contact = _segment(save_time - 0.16, save_time + 0.38, 0.15, "goal_front", fps)
    recovery = _segment(save_time + 0.30, end, 1.0, "wide_goal", fps)
    finale = tuple(_Frame(end, "wide_goal") for _ in range(round(1.4 * fps)))
    ...
    return (
        ...
        _Clip("CONTINUOUS PASS → HIGH STRIKE → LATE-LUNGE SAVE", continuous),
        _Clip(
            f"DATA-DRIVEN LEG PULSE · {metrics['lunge_span_m']:.3f} m LUNGE · NOT AIRBORNE",
            lunge,
        ),
        _Clip(
            f"TRUE RIGHT-GLOVE CONTACT · {surface_mm:+.2f} mm SURFACE DISTANCE · NO GOAL",
            contact,
        ),
        _Clip(
            f"POST-SAVE BRAKE · TAIL SPEED {metrics['recovery_maximum_linear_speed_mps']:.3f} m/s",
            recovery,
        ),
