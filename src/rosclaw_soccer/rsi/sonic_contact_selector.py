...
def choose_lateral(model: dict[str, Any], *, ball_x_m: float, ball_y_m: float) -> float:
    ...
    if claimed != hash_json(body) or model.get("schema") not in (SCHEMA, SCHEMA_V2, SCHEMA_V3):
        raise ValueError("selector checkpoint integrity or schema failed")
    maximum_x = {SCHEMA: 1.9, SCHEMA_V2: 2.1, SCHEMA_V3: 2.2}[model["schema"]]
    if (
    ...
    if model["schema"] in (SCHEMA_V2, SCHEMA_V3):
        ...
        if model["schema"] == SCHEMA_V3:
            ...
            if (
                ...
                or [knot[0] for knot in knots] != [0.04, 0.08, 0.12, 0.16]
                or any(
                    not isinstance(knot[1], float) or not 0.0 <= knot[1] <= 0.1
                    for knot in knots
                )
            ):
