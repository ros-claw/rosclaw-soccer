"""Teacher-free causal canary for an adaptive target-velocity specialist."""
...
def run_adaptive_target_velocity_canary(
    *,
    asset_root: Path,
    source_s95_dir: Path,
    ...
    if not 1 <= workers <= 3:
        raise ValueError("adaptive target canary workers must be in [1, 3]")
    ...
    if (
        teacher.get("schema_version") != "rosclaw.growth.adaptive_target_teacher_discovery.v1"
        or teacher.get("status") != "REJECTED_ADAPTIVE_TARGET_TEACHER_DISCOVERY"
        or training.get("status") != "PASS_ADAPTIVE_TARGET_VELOCITY_DISTILLATION"
        or training.get("source_discovery_report_hash") != teacher["report_hash"]
        ...
        raise ValueError("adaptive target canary lineage changed")
    teacher_successes = [row for row in teacher["rows"] if row["quality"]["chain_passed"]]
    if len(teacher_successes) != 1:
        raise ValueError("adaptive target canary expects one isolated teacher success")
    source_row = teacher_successes[0]
    ...
    if hash_bytes(source_trajectory.read_bytes()) != source_artifact["file_hash"]:
        raise ValueError("adaptive target canary source trajectory changed")
    qualification = qualify_g1_assets(asset_root)
    qualification.require_eligible()
    if actor.body_hash != qualification.body_hash:
        raise ValueError("adaptive target canary Body identity changed")
    ...
    if not handoff_decision.accepted or handoff_decision.handoff_policy_frame is None:
        raise ValueError("adaptive target canary handoff rejected the source action")
    ...
    request: dict[str, Any] = {
        "schema_version": "rosclaw.growth.adaptive_target_velocity_canary_request.v1",
    ...
    speed_gain = float(
        candidate_result["shot_peak_ball_speed_mps"]
        - baseline["result"]["shot_peak_ball_speed_mps"]
    )
    gates = {
        "candidate_safe": bool(candidate_quality["safe"]),
        "strict_right_foot_chain": bool(candidate_quality["strict_chain_passed"]),
        "goal_scored": bool(candidate_result["goal_crossed"]),
        "minimum_shot_speed": candidate_result["shot_peak_ball_speed_mps"] >= 9.0,
        "exact_replay": exact_replay,
        "teacher_absent": all(not run["teacher_active"] for run in runs),
        "actor_executed": primary["actor_active"] and replay["actor_active"],
        "causal_gain_over_no_actor": (
            not baseline["quality"]["strict_chain_passed"] and speed_gain >= 1.0
        ),
    }
    report: dict[str, Any] = {
        "schema_version": "rosclaw.growth.adaptive_target_velocity_canary.v1",
        "status": (
            "PASS_ADAPTIVE_TARGET_VELOCITY_CANARY"
            if all(gates.values())
            else "REJECTED_ADAPTIVE_TARGET_VELOCITY_CANARY"
        ),
        ...
        "metrics": {
            ...
            "baseline_shot_speed_mps": baseline["result"]["shot_peak_ball_speed_mps"],
            "shot_speed_gain_mps": speed_gain,
...
def _run(
    ...
    active = np.asarray(trajectory["shooter_target_velocity_contact_actor_active"], dtype=np.bool_)
...
def validate_adaptive_target_velocity_canary(path: Path) -> dict[str, Any]:
    source = path.expanduser().resolve()
    report = _bound_json(source)
    if (
        report.get("status") != "PASS_ADAPTIVE_TARGET_VELOCITY_CANARY"
        ...
        raise ValueError("adaptive target canary authority is invalid")
    if hash_bytes((source.parent / "request.json").read_bytes()) != report["request_hash"]:
        raise ValueError("adaptive target canary request changed")
    for run in report["runs"]:
        artifact = run["trajectory"]
        path = source.parent / artifact["file"]
        if hash_bytes(path.read_bytes()) != artifact["file_hash"]:
            raise ValueError("adaptive target canary trajectory changed")
    return report
...
def _new_external_output(path: Path) -> Path:
    ...
    if output.exists() or output == checkout or checkout in output.parents:
        raise ValueError("adaptive target canary output must be new and external")
