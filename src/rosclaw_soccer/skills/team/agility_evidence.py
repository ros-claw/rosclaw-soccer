"""Strict SIM_ONLY evidence for joint-group agility growth."""
...
class G1AgilityEvidence:
    ...
    strict_replay: bool
    search: G1AgilitySearchResult
    result: dict[str, Any]
    candidate_naturalness: G1MotionNaturalnessMetrics
    candidate_agility: G1AgilityMetrics
    ...
    schema_version: str = "rosclaw_soccer.g1_agility_evidence.v1"
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            **asdict(self),
            ...
            "candidate_agility": asdict(self.candidate_agility),
            "claims": {
...
def run_g1_agility_development(
    *,
    asset_root: Path,
    ...
    """Search, strictly replay, and persist one robust agility candidate."""
    ...
    if output == checkout or checkout in output.parents:
        raise ValueError("agility evidence must be outside the source checkout")
    ...
    search = search_g1_agility_candidate(
    ...
    if search.selected_candidate is None or search.selected_trial is None or not search.passed:
        raise RuntimeError("agility search found no robust eligible candidate")
    if search.neighborhood is None:
        raise RuntimeError("agility search passed without a local neighborhood")
    ...
    if result.pass_contact_time_sec is None or result.shot_contact_time_sec is None:
        raise RuntimeError("agility replay is missing a required contact time")
    ...
    request = {
        "schema_version": "rosclaw_soccer.g1_agility_request.v1",
        ...
        "neighborhood_eligible_fraction": search.neighborhood.eligible_fraction,
    ...
    passed = bool(
        ...
        and search.neighborhood is not None
        and search.neighborhood.passed
    ...
    _write_json(output / "g1-agility-growth.json", evidence.to_dict())
    return evidence
...
__all__ = ["G1AgilityEvidence", "run_g1_agility_development"]
